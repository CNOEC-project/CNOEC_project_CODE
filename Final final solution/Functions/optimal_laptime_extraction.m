function time_vec = optimal_laptime_extraction(z_full,u_vec,N,chassis,tyre,s_col,z_norm_factor,u_norm_factor,k_col,B_col,nz_grid_points,d_th)
    %OPTIMAL_LAPTIME_EXTRACTION - extracts the time instant in the optimal laptime at each point of the vehicle  
    %
    ...
    for k = 0:N-1
        ...
        for jj=1:d_th
            [~, ~, tprime_kj(1,jj)] = vehicle_dynamics(z_col(:,d_th*k+jj), u(:,k+1), chassis,tyre, k_col(:,d_th*k+jj));
        end
