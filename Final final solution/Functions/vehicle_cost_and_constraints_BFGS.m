function v = vehicle_cost_and_constraints_BFGS(z_full,u_vec,N,chassis,tyre,s_col,z_norm_factor,u_norm_factor,k_col,C_col,D_col,nz_grid_points,d_th,B_col)
    ...
    z_col  = reshape(z_col_vec_norm, n_states, d_th*N).*z_norm_factor;     % Reshape z_col_vec_norm into a matrix  
    ...
    tprime_kj = zeros(1,d_th);                                             % Initialize the vector of time variation at collocation points
    ...
    % Forces for the friction ellipse 
    [Fz_f,Fz_r,Fx_f,Fx_r,Fy_f,Fy_r,mu_f,mu_r] = friction_ellipse_forces(z_grid,u_grid,chassis,tyre);
    
    dXkj = zeros(n_states,d_th);                                           % Initialize the vector of state derivatives at collocation points

    g = [T_drive_vec.*T_brake_vec];                                        % Nonlinear equality constraint matrix - initialization 
    
    % Nonlinear inequality constraint matrix - normalized friction ellipse
    h = [(mu_f.^2-(Fx_f.^2+Fy_f.^2)./Fz_f.^2)'/norm(mu0_x + pD2*(Fz_f-Fz0)/Fz0);  % Front wheel                              
        (mu_r.^2-(Fx_r.^2+Fy_r.^2)./Fz_r.^2)'/norm(mu0_x + pD2*(Fz_r-Fz0)/Fz0)];  % Rear wheel                              

    for kk = 0:N-1
        % Concatenate normalized states
        Z = [z_grid(:,kk+1)./z_norm_factor       z_col(:,d_th*kk+(1:d_th))./z_norm_factor]; 
        ...
        for ii=1:d_th
            [dXkj_not_normalized, ~, tprime_kj(1,ii)] = vehicle_dynamics(z_col(:,d_th*kk+ii), u_grid(:,kk+1),chassis,tyre, k_col(:,d_th*kk+ii));
