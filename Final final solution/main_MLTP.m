...
title('Circuit centerline','FontSize',16);
xlabel('$X [m]$','Interpreter','LaTex','FontSize',16);
ylabel('$Y [m]$','Interpreter','LaTex','FontSize',16);
legend('Centerline','FontSize',14);
grid on;
set(gca, 'FontSize', 16);
...
omega_r0    =       vx0/tyre.rw;   % Initial guess - rear wheel rotational speed (rad/s)
...
N_col = d_th*N;                                                           % Total number of collocation points
xc0 = reshape(kron(z0(:,1:end-1),ones(1,d_th)),d_th*N*n_states,1);        % Constant interpolation between z0
...
lb_xc_vec= repmat(lb_z_normalized,N*d_th,1);
ub_xc_vec= repmat(ub_z_normalized,N*d_th,1);
...
beq(1) = 1e-2;    % To help the solver, we ask a not exactly null initial velocity(i.e. to avoid tprime = inf) 
...
p = (N+1) + d_th*n_states*N + n_states*N;              % Number of nonlinear equality constraints
q = (N+1)*2;                                           % Number of nonlinear inequality constraints
...
% Run the solver
tic;
[x_opt,f_val,niter,exitflag,xsequence] = myfmincon(@(x) vehicle_cost_and_constraints_BFGS([x(1:nz_grid_points); x(nz_grid_points+nu_grid_points+1:end)],x(length(z0_vec)+1:length(z0_vec)+length(u0_vec)),N,chassis,tyre,s_col,z_norm_factor,u_norm_factor,k_col,C_col,D_col,nz_grid_points,d_th,B_col), ...
...
z_col_opt_vec  = x_opt(nz_grid_points+nu_grid_points+1:end);             % Normalized inputs at the collocation points

total_lap_time_opt   = sqrt(f_val);                                      % Optimal lap time [s]                    
laptime_grid_vec_opt = optimal_laptime_extraction([z_grid_opt_vec;  z_col_opt_vec],u_opt_vec,N,chassis,tyre,s_col,z_norm_factor,u_norm_factor,k_col,B_col,nz_grid_points,d_th);
laptime_vec_opt_full = interp1(s_grid', laptime_grid_vec_opt', s_full, 'linear')';  % Optimal time instant at the collocation and grid points [s]
...
z_col_opt = reshape(z_col_opt_vec,n_states,d_th*N).*z_norm_factor;       % States at the collocation points

% Extract the full state vector (at grid and collocation points)
z_full = kron(z_grid_opt(:,1:end-1), [1 zeros(1,d_th)]) + reshape([zeros(n_states,N); reshape(z_col_opt,n_states*d_th,N)],n_states,[]);
z_full(:,end+1) = z_grid_opt(:,end);

% Extract the full input vector (at grid and collocation points)
u_full = interp1(s_grid', u_opt', s_full, 'previous')';              % Constant inputs between grid points

% States
vx_full_opt         =       z_full(1,:);                             % Optimal body x velocity (m/s) - at grid and collocation points
vy_full_opt         =       z_full(2,:);                             % Optimal - body y velocity (m/s) - at grid and collocation points
omega_full_opt    =         z_full(3,:);                             % Optimal - yaw rate (rad/s) - at grid and collocation points
n_full_opt          =       z_full(4,:);                             % Optimal - transversal displacement (m) - at grid and collocation points
epsi_full_opt       =       z_full(5,:);                             % Optimal - heading angle (rad) - at grid and collocation points
omega_f_full_opt    =       z_full(6,:);                             % Optimal - front wheel rotational speed (rad/s) - at grid and collocation points
omega_r_full_opt    =       z_full(7,:);                             % Optimal - rear wheel rotational speed (rad/s) - at grid and collocation points

% Inputs
T_drive_opt_full         =  u_full(1,:);                             % Optimal driving torque (Nm) - at grid and collocation points
T_brake_opt_full         =  u_full(2,:);                             % Optimal braking torque (Nm) - at grid and collocation points
delta_opt_full           =  u_full(3,:);                             % Optimal steering angle (rad) - at grid and collocation points
