...
if strcmp(myoptions.Hessmethod,'GN')    % Constrained Gauss-Newton method
    ...
    while  k < myoptions.nitermax && (norm(gradLagr) > myoptions.tolgrad...
        ...
        [pk,~,~,~,LagMult]  =   quadprog(Hk,gradfxk,-gradhk',hxk,gradgk',-gxk,[],[],xk,myoptions.QPoptions);
        if isempty(pk) || isempty(LagMult)
            myoptions.QPoptions.Algorithm = 'active-set';
            [pk,~,~,~,LagMult]  =   quadprog(Hk,gradfxk,-gradhk',hxk,gradgk',-gxk,[],[],xk,myoptions.QPoptions);
            myoptions.QPoptions.Algorithm = 'interior-point-convex';
        end
