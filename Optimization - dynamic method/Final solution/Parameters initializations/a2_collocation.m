...
OPT_ds = 70;                        % Collocation step (m)
OPT_d = 3;                          % Degree of interpolating polynomials

% Collocation constants - from literature
...
dsk = diff(s_grid); % Note that the size of s_knot is N+1 whereas for dsk it is N

% Value of the independent variable at the collocation points
s_col = kron(dsk,tau_col)+kron([0 cumsum(dsk(1:end-1))],ones(1,OPT_d)); %  Value of s at the collocation points (in between grid points)

% Full array of points including grid points and collocation points
s_full = kron(s_grid(1:end-1), [1 zeros(1,OPT_d)]) + reshape([zeros(1,N); reshape(s_col,OPT_d,N)],1,[]); % Collect all values of the independent variable at the grid points and collocation points
