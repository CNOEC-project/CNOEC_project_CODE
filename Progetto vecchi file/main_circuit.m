...
j=2;
for i=2:h-1
   while sqrt((x_out7(j+1)-x_outer(i-1))^2+(y_out7(j+1)-y_outer(i-1))^2)<4.427 && j<length(x_out7)-1
       j=j+1;
   end
   x_outer(i)=x_out7(j);
   y_outer(i)=y_out7(j);
...
k=2;
for i=2:h-1
   while sqrt((x_in7(k+1)-x_inner(i-1))^2+(y_in7(k+1)-y_inner(i-1))^2)<4.3237 && k<length(x_in7)-1
       k=k+1;
   end
   x_inner(i)=x_in7(k);
   y_inner(i)=y_in7(k);
...
% figure();
% plot(x_inner,y_inner,'Blue',x_outer,y_outer,'red'),grid on;
% 
...
% %% Test mezzeria
% alpha=0.3*ones(length(x_inner),1);
% 
...
% k=1;
% for i=1:531
%     if sqrt((x_outer(i)-x_in7(k))^2+(y_outer(i)-y_in7(k))^2)<12
%     x_inner(i)=x_in7(k);
%     y_inner(i)=y_in7(k);
%     k=k+1;
%     else
%          while sqrt((x_outer(i)-x_in7(k))^2+(y_outer(i)-y_in7(k))^2)>12 && k<length(x_in7)-1
%               k=k+1;
%          end
%          x_inner(i)=x_in7(k);
%          y_inner(i)=y_in7(k);
