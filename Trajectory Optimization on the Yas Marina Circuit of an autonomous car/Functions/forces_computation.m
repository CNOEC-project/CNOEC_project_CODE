function [F_drag,F_lift,Fz_f,Fz_r,Fx_f,Fx_r,Fy_f,Fy_r] = forces_computation(z,u,chassis,tyre)

%FORCES_COMPUTATION Computes the force acting on the vehicle at the current
% iteration.
% We consider 8 forces acting simultaneously on the vehicle: 
...
%
% Inputs:   z               (States values at a given point)
%           u               (Input values at a given point)
...
Fx_f = Dx_f*sin(cx*atan(bx*sx_f-ex*(bx*sx_f-atan(bx*sx_f))));   % Front longitudinal tyre-road force (N)
Fx_r = Dx_r*sin(cx*atan(bx*sx_r-ex*(bx*sx_r-atan(bx*sx_r))));   % Rear longitudinal tyre-road force (N)
